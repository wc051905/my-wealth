...
$(function() {
    ...
    //设置时间框缓存
    $('.time-input').attr('autocomplete', 'off');
...
var closeItem = function(dataId){
	...
	if($.common.isNotEmpty(dataId)){
	    ...
	    // 移除相应tab对应的内容区
	    $('.mainContent .Ait_iframe[data-id="' + dataId + '"]', topWindow).remove();
	    return;
	}
	...
	if($.common.isNotEmpty(panelUrl)){
	    ...
	    $('.mainContent .Ait_iframe', topWindow).each(function() {
	        if ($(this).data('id') == panelUrl) {
	            $(this).show().siblings('.Ait_iframe').hide();
	            return false;
            }
        });
    }
...
function createMenuItem(dataUrl, menuName, isRefresh) {
    ...
    if (flag) {
        ...
        // 添加选项卡对应的iframe
        var str1 = '<iframe class="Ait_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" data-panel="' + panelUrl + '" seamless></iframe>';
        $('.mainContent', topWindow).find('iframe.Ait_iframe').hide().parents('.mainContent').append(str1);
...
function activeWindow() {
    ...
    return $('.Ait_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
...
function downloadFile(obj){
    ...
    var fileUrl = $(obj).attr("data");
    window.location.href = encodeURI(ctx + "common/downloadNew?fileName=" + fileName + "&fileUrl=" + fileUrl + "&delete=" + false);
}

function downloadFileList(fileName, fileUrl){
    window.location.href = encodeURI(ctx + "common/downloadNew?fileName=" + fileName + "&fileUrl=" + fileUrl + "&delete=" + false);
}

// 全选操作
function checkAll(obj){
    $(obj).parent().parent().find("input").prop("checked", $(obj).prop("checked"));
}
...
function loadDateInput(){
    layui.use('laydate', function () {
        var laydate = layui.laydate;
        $.each($(".date-input"), function (i, object) {
            laydate.render({
                elem: '#' + $(object).attr("id"),
                type: 'date',
                format: 'yyyy-MM-dd'
...
function getMinNum(arrNum){
    ...
    var minNum = Math.min.apply(null, arrNew);
    var maxNum = Math.max.apply(null, arrNew);
    minNum = minNum - minNum % 10;
    if(maxNum - minNum < 10){
        minNum = minNum - 10;
    }
    if(minNum < 0){
...
function getXData(dataValue,dataYear){
    var date=new Date;
    var year=date.getFullYear();
    var month=date.getMonth()+1;
    if(dataYear == 1 || year == dataYear){
        if(month == 1){
            return [dataValue[0]];
        }else if(month == 2){
            return [dataValue[0],dataValue[1]];
        }else if(month == 3){
            return [dataValue[0],dataValue[1],dataValue[2]];
        }else if(month == 4){
            return [dataValue[0],dataValue[1],dataValue[2],dataValue[3]];
        }else if(month == 5){
            return [dataValue[0],dataValue[1],dataValue[2],dataValue[3],dataValue[4]];
        }else if(month == 6){
            return [dataValue[0],dataValue[1],dataValue[2],dataValue[3],dataValue[4],dataValue[5]];
        }else if(month == 7){
            return [dataValue[0],dataValue[1],dataValue[2],dataValue[3],dataValue[4],dataValue[5],dataValue[6]];
        }else if(month == 8){
            return [dataValue[0],dataValue[1],dataValue[2],dataValue[3],dataValue[4],dataValue[5],dataValue[6],dataValue[7]];
        }else if(month == 9){
            return [dataValue[0],dataValue[1],dataValue[2],dataValue[3],dataValue[4],dataValue[5],dataValue[6],dataValue[7],dataValue[8]];
        }else if(month == 10){
            return [dataValue[0],dataValue[1],dataValue[2],dataValue[3],dataValue[4],dataValue[5],dataValue[6],dataValue[7],dataValue[8],dataValue[9]];
        }else if(month == 11){
            return [dataValue[0],dataValue[1],dataValue[2],dataValue[3],dataValue[4],dataValue[5],dataValue[6],dataValue[7],dataValue[8],dataValue[9],dataValue[10]];
        }else if(month == 12){
            return [dataValue[0],dataValue[1],dataValue[2],dataValue[3],dataValue[4],dataValue[5],dataValue[6],dataValue[7],dataValue[8],dataValue[9],dataValue[10],dataValue[11]];
        }
    }else{
        return dataValue;
    }
}
function getDateArray(dataYear){
    var date=new Date;
    var year=date.getFullYear();
    var month=date.getMonth()+1;
    if(dataYear == 1 || year == dataYear){
...
function composeTime(val){
    ...
    if(temp.length == 1){
        ...
        if(temp.substring(0,2) >= 24){
            temp = "00" + ":" + temp.substring(2,4);
        }else{
            temp = temp.substring(0,2) + ":" + temp.substring(2,4);
        }
    }
    var timeP = /^([0-1]?[0-9]|2[0-3]):([0-5][0-9])$/;
    if(!timeP.test(temp)){
        temp = '00:00';
    }
...
function composeTimeReturn(obj){
...
// 多人筛选
function selectMultipleUser(){
    $.modal.openOptions({
        title: "多人筛选",
        height: 500,
        url: "/select/user/selectMultipleUser",
        callBack: function (index, layero) {
            var iframeWin = layero.find('iframe')[0];
            $("#empIds").val(iframeWin.contentWindow.getResult());
            $.table.search();
            $.modal.close(index);
        }
    });
...
function onEditableSaveCommon (field, row, oldValue, $el) {
