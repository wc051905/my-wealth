    ...
    $.extend({
        ...
        table: {
            ...
            init: function(options) {
                ...
                $('#' + options.id).bootstrapTable({
                    ...
                    toolbarAlign: options.toolbarAlign,                     // 指定工作栏
            ...
            //字数超过指定数下拉按钮切换
            dropdownToggleSum: function (value) {
                var actions = [];
                actions.push('<div class="btn-group">');
                actions.push('<button type="button" class="btn btn-xs dropdown-toggle" data-toggle="dropdown" aria-expanded="false">');
                actions.push('<i class="fa fa-cog"></i>&nbsp;<span class="fa fa-chevron-down"></span></button>');
            ...
            // 下载自定义模板
            importTemplateSpecial: function() {
                table.set();
                window.location.href = activeWindow().table.options.importTemplateUrl;
            },
            // 导入数据
            ...
            selectDictLabelNoStyle: function(datas, value) {
                ...
                $.each(datas, function(index, dict) {
                    if (dict.dictValue == ('' + value)) {
                        actions.push($.common.sprintf("<span class='%s'>%s</span>", "", dict.dictLabelLanguage));
                        return false;
                    }
                });
                return actions.join('');
            ...
            selectDictLabel: function(datas, value) {
                ...
                $.each(datas, function(index, dict) {
                    if (dict.dictValue == ('' + value)) {
                        ...
                        actions.push($.common.sprintf("<span class='%s'>%s</span>", listClass, dict.dictLabelLanguage));
                        return false;
                    }
                });
                if (actions.length === 0) {
             ...
             * */
            // 回显法人
            selectCpnyLabel: function(datas, value) {
                var actions = [];
                $.each(datas, function(index, cpny) {
                    if (cpny.companyId == ('' + value)) {
                        var listClass = $.common.equals("default", cpny.listClass) || $.common.isEmpty(cpny.listClass) ? "" : "badge badge-" + cpny.listClass;
                        actions.push($.common.sprintf("<span class='%s'>%s</span>", listClass, cpny.companyName));
                        return false;
                    }
                });
                return actions.join('');
        ...
        modal: {
            ...
            openOptions: function (options) {
                ...
                var _rowLength = options.rowLength;

                if ($.common.isEmpty(options.yes)) {
                    options.yes = function(index, layero) {
                        options.callBack(index, layero, _obj, _param, _rowLength);
                    }
                }
        ...
        operate: {
            ...
            toImport: function() {
                ...
                var rowErrorFlag = $.table.selectColumns(table.options.errorFlag);
                if(rowErrorFlag.indexOf("Y") >= 0){
                    $.modal.alertWarning("选中项中包含错误项，请更改后再进行导入");
                    return;
                }
                if (rows.length == 0) {
                    $.modal.alertWarning("请至少选择一条记录");
                ...
                $.modal.confirm("确认要导入选中的" + rows.length + "条数据吗?", function() {
                    var url = table.options.toImportUrl;
            ...
            withdrawAll: function() {
                ...
                $.modal.confirm("确认要撤回选中的" + rows.length + "条数据吗?", function() {
                    var url = table.options.withdrawUrl;
                    var data = { "ids": rows.join() };
                    $.operate.submit(url, "post", "json", data);
                });
            },
            affirmAll: function(applyType, affirmFlag) {
                table.set();
                var rows = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
                if (rows.length == 0) {
                    $.modal.alertWarning("请至少选择一条记录");
                ...
                var affirmFlagName = '否决';
                var url = table.options.affirmUrl;
                if(affirmFlag == 'pass'){
                    affirmFlagName = '通过';
                    $.modal.confirm("确认要" + affirmFlagName + "选中的" + rows.length + "条数据吗?", function() {
                        var data = { "ids": rows.join(),"affirmFlag": affirmFlag, "applyType": applyType};
                        $.operate.submit(url, "post", "json", data);
                    });
                } else{
                    var data = { "ids": rows.join(),"affirmFlag": affirmFlag, "applyType": applyType};
                    $.modal.open('否决原因', "/mss/affirmLeave/resonPopUps?ids=" + rows.join() + "&affirmFlag=" + affirmFlag + "&applyType=" + applyType + "&url=" + url,'600', '300');
                }
            },
            // 批量删除信息
            removeAll: function() {
            ...
            //弹窗添加取消按钮
            addNoBtn: function(id) {
                table.set();
                $.modal.openNoBTn("添加" + table.options.modalName, $.operate.addUrl(id));
            },
            // 添加信息，以tab页展现
            addTab: function (id) {
                table.set();
            ...
            editNoBtn: function(id) {
                ...
                if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
                    ...
                    var url = table.options.updateUrl.replace("{id}", row[table.options.uniqueId]);
                    $.modal.openNoBTn("修改" + table.options.modalName, url);
                } else {
                    $.modal.openNoBTn("修改" + table.options.modalName, $.operate.editUrl(id));
                }
            },
            // 修改信息，以tab页展现
            editTab: function(id) {
            ...
            search: function(id) {
                ...
                if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
                    ...
                    var url = table.options.searchUrl.replace("{id}", row[table.options.uniqueId]);
                    $.modal.openNoBTn("审批" + table.options.modalName, url);
                } else {
                    $.modal.openNoBTn("查看" + table.options.modalName, $.operate.searchUrl(id));
                }
            },
            // 修改访问地址
            searchUrl: function(id) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = table.options.searchUrl.replace("{id}", id);
                } else {
                    var id = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
                    if (id.length == 0) {
                        $.modal.alertWarning("请至少选择一条记录");
            ...
            //自定义保存失败提示
            saveCust: function(url, data, callback) {
                var config = {
                    url: url,
                    type: "post",
                    dataType: "json",
            ...
            /*自定义保存失败提示*/
            saveCustTwo: function(url, data, hint, callback) {
                var config = {
                    url: url,
                    type: "post",
                    dataType: "json",
            ...
            /*自定义保存失败提示*/
            saveTabCust: function(url, data, hint, callback) {
                var config = {
                    url: url,
                    type: "post",
                    dataType: "json",
            ...
            saveReloadParent: function(url, data, callback) {
                var config = {
                    url: url,
                    type: "post",
                    dataType: "json",
                    ...
                    success: function(result) {
                        ...
                        var target = $('.Ait_iframe[data-id="' + currentId + '"]', topWindow);
            ...
            successTabCallback: function(result) {
                if (result.code == web_status.SUCCESS) {
                    ...
                    var $contentWindow = $('.Ait_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
                    $.modal.close();
        ...
        tree: {
            ...
            // 获取当前被勾选集合
            getCheckedNodeIds: function(column) {
                var _column = $.common.isEmpty(column) ? "id" : column;
                var nodes = $._tree.getCheckedNodes(true);
                return $.map(nodes, function (row) {
                    return row[_column];
            ...
            // 获取当前被勾选集合
            getCheckedNodeNames: function(column) {
            ...
            // 获取当前被勾选集合
            getCheckedNodeIdNames: function(column) {
                var _column = $.common.isEmpty(column) ? "name" : column;
                var _column1 = $.common.isEmpty(column) ? "id" : column;
                var nodes = $._tree.getCheckedNodes(true);
                return $.map(nodes, function (row) {
                    return row[_column1] + row[_column];
                }).join();
            },
            // 不允许根父节点选择
        ...
        common: {
            ...
            dictToSelect: function(datas, value, name) {
                ...
                $.each(datas, function(index, dict) {
                    ...
                    actions.push($.common.sprintf(">%s</option>", dict.dictLabelLanguage));
                });
                actions.push('</select>');
                return actions.join('');
            },
