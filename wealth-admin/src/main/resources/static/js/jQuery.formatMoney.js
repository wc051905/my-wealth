...
;(function($)
{
    $.extend({
        ...
        formatMoneySimple:function(sVal){
            if (sVal == 0 || typeof(sVal) == undefined) {
                return 0;
            }else{
                if(sVal){
                    var fTmp = sVal.toFixed(0);
                    return fTmp.replace(/(\d{1,3})(?=(\d{3})+(?:$|\.))/g,'$1,');
                }else{
                    return 0;
                }
            }
        },
        formatNumber:function(sVal, fix){
            if (sVal == 0 || typeof(sVal) == undefined || sVal== null || sVal=='') {
                return 0;
            }else{
                if(sVal && !isNaN(sVal)){
                    var fTmp = parseFloat(sVal).toFixed(fix);
                    return fTmp.replace(/(\d{1,3})(?=(\d{3})+(?:$|\.))/g,'$1,');
                }else{
                    return 0;
                }
